...
client.on('message', message => {
  ...
  client.user.setActivity(`Do for help | g+help `);
  ...
  if(message.content === "This is so cute") {
        message.channel.send("Really, xD");
    }
     if (message.content === 'xD') {
    message.channel.send('too laughing, xD'); 
     }
  if  ( message.content === 'Andrea Pasculli#8957') {
    
       message.reply("What do you have to question to my creator?");
}
   if  ( message.content === 'fakeban') {
    
       message.channel.send("'**Riccardo111**' was banned for '**SPAM**'");
}
        if(message.content === "g+purge") {
   ...
   if (message.content === 'g+info') {
    message.reply(' :smiley:| Gimo 1.2.4 | Created by Lxi Inc. Dev | Programmed by @Andrea Pasculli#8957 | :smiley:'); 
  }
  ...
  //Bot Commands
  //------------------------------
  if(message.content === "g+SayHelloAdmins") {
    // Most of this command is identical to kick, except that here we'll only let admins do it.
    // In the real world mods could ban too, but this is just an example, right? ;)
  ...
  if (message.content === 'a scalare una montagna') {
   ...
   message.reply(' | You can use: __**g+ban**__ for ban a player, and the __**all**__ commands of a normal discord bot. | g+info for the **informations** of LxiBot | WG Commands: g+WGJoin - g+WGChat - g+WGInfo | LxiCraft Commands: lc+serverinfo - lc+serverthreads | ');
   }
...
client.login("NDQ5OTI3NzI4OTc2NzU2NzM3.Derz5A.4IpTUgTixi1SD4Ld_Tk2etzLgkk")